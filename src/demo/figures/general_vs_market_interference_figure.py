...
if __name__ == "__main__":
    ...
    def draw_node(ax, pos, label, color=NODE_FILL_COLOR):
        ...
        circle = Circle(
            pos, 0.25, color=color, ec=NODE_EDGE_COLOR, linewidth=2, zorder=3
        )
        ax.add_patch(circle)
        ax.text(
            pos[0],
    ...
    def draw_arrow(ax, start, end, color):
        ...
        # Draw the line
        ax.plot(
            [start[0], end[0]], [start[1], end[1]], color=color, linewidth=2.5, zorder=1
        )
        ...
        p1 = (tip_x, tip_y)  # Tip
        p2 = (
            tip_x - dx_norm * head_length + perp_x * head_width,
    ...
    def draw_dotted_line(ax, start, end):
        ...
        ax.plot([start[0], end[0]], [start[1], end[1]], "k:", linewidth=1.5, zorder=2)
    ...
    def draw_bracket(ax, x, y_top, y_bottom, label=""):
        ...
        ax.plot(
            [x_bracket, x_bracket + bracket_width], [y_top, y_top], "k-", linewidth=2
        )
        ax.plot([x_bracket, x_bracket], [y_top, y_bottom], "k-", linewidth=2)
        ax.plot(
            [x_bracket, x_bracket + bracket_width],
            [y_bottom, y_bottom],
            "k-",
            linewidth=2,
        )
    ...
    fig.suptitle(
        "General vs Market Interference", fontsize=24, fontweight="bold", y=0.96
    )
    ...
    # Save the figure
    fig.savefig(
        get_figures_path("general-vs-market-interference.png"),
        bbox_inches="tight",
        pad_inches=0.1,
    )
    plt.close()
