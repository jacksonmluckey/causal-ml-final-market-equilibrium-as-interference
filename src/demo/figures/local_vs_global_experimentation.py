...
df = (
    pl.read_csv(get_data_path("local_vs_global_experiment.csv"))
    # add cumulative sum
    .with_columns(
        pl.col("U").cum_sum().over("experimentation").alias("cumulative_utility")
    )
    ...
    .with_columns(
        ...
        pl.col("variable").cast(
            pl.Enum(["U", "p", "cumulative_regret", "cumulative_utility"])
        ),
        ...
        .cast(
            pl.Enum(
                [r"High ($d_a = 0.7$)", r"Medium ($d_a = 0.5$)", r"Low ($d_a = 0.3$)"]
            )
        )
...
(
    ...
    + gg.facet_grid(
        ...
        rows="variable",
        scales="free_y",
        labeller=gg.labeller(experimentation=exp_labeller, variable=var_labeller),
    )
    +
    ...
    width=6, height=8, filename=get_figures_path("global_vs_local_experimentation.png")
)
