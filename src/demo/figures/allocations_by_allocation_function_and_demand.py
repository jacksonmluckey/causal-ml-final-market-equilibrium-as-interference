from demo.method import create_queue_allocation, create_linear_allocation
import numpy as np
...
def allocations_by_function_and_demand():
    ...
    allocation_df = pl.DataFrame(
        ...
        schema=[
            "allocation_function",
            "demand_per_active_supplier",
            "allocation_per_supplier",
            "omega_derivative",
            "queue_capacity",
        ],
    ).with_columns(
        ...
        .then(pl.concat_str([pl.lit(r"$L=$"), pl.col.queue_capacity, pl.lit("")]))
