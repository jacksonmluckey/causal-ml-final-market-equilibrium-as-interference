...
if __name__ == "__main__":
    ...
    # Compare different allocation functions
    ...
    smooth = SmoothLinearAllocation()
    
    plt.figure(figsize=(10, 6))
    
    plt.plot(x_vals, [queue_L4.omega(x) for x in x_vals], label='Queue L=4')
    plt.plot(x_vals, [queue_L8.omega(x) for x in x_vals], label='Queue L=8')
    plt.plot(x_vals, [queue_L15.omega(x) for x in x_vals], label='Queue L=15')
    plt.plot(x_vals, [linear.omega(x) for x in x_vals], '--', label='Linear')
    plt.plot(x_vals, [smooth.omega(x) for x in x_vals], ':', label='Smooth')
    
    plt.xlabel('x = d/t (demand per supplier)')
    plt.ylabel('ω(x) (allocation per supplier)')
    plt.title('Regular Allocation Functions')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('/home/claude/market_model/allocation_functions.png', dpi=150)
    plt.close()
    
    print("Allocation function values at x=0.5:")
    print(f"  Queue L=8: ω(0.5) = {queue_L8.omega(0.5):.4f}")
    print(f"  Linear: ω(0.5) = {linear.omega(0.5):.4f}")
    
    print("\nAllocation function values at x=1.5:")
    print(f"  Queue L=8: ω(1.5) = {queue_L8.omega(1.5):.4f}")
    print(f"  Linear: ω(1.5) = {linear.omega(1.5):.4f}")
    
    print("\nDerivatives at x=0.5:")
    print(f"  Queue L=8: ω'(0.5) = {queue_L8.omega_derivative(0.5):.4f}")
    print(f"  Smooth: ω'(0.5) = {smooth.omega_derivative(0.5):.4f}")