...
if __name__ == "__main__":
    ...
    # Create logistic choice function
    choice_fn = LogisticChoice(alpha=1.0)
    
    # Create log-normal cost distribution (as in the paper)
    # log(B_i/20) ~ N(0, 1)
    cost_dist = LogNormalCosts(log_mean=0.0, log_std=1.0, scale=20.0)
    
    population = SupplierPopulation(choice_fn, cost_dist)
    
    # Plot activation probability as function of expected revenue
    revenues = np.linspace(0, 100, 100)
    probs = [population.activation_probability(r) for r in revenues]
    
    plt.figure(figsize=(10, 6))
    plt.plot(revenues, probs)
    plt.xlabel('Expected Revenue (p × E[Ω])')
    plt.ylabel('Activation Probability μ')
    plt.title('Supplier Activation vs Expected Revenue')
    plt.grid(True, alpha=0.3)
    plt.savefig('/home/claude/market_model/activation_probability.png', dpi=150)
    plt.close()
    
    print("Activation probabilities:")
    for r in [10, 20, 30, 40, 50]:
        print(f"  Revenue={r}: μ = {population.activation_probability(r):.3f}")
    
    # Test sampling
    n = 1000
    payments = np.full(n, 30.0)  # Everyone gets payment 30
    Z = population.sample_suppliers(n, payments, expected_allocation=1.0)
    print(f"\nSample: {Z.sum()}/{n} suppliers activated (expected: {population.activation_probability(30)*n:.0f})")