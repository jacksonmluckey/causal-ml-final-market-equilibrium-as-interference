---
title: "demo Implementation of Market Equilibrium as Interference"
format:
    html:
...
demand_model = DemandParameters(states, concentration_param=50)
```
...
for n in [1, 10, 100, 1000, 10000, 100000]:
    for state_name, state in states.items():
        print(f'n={n}')
        samples = [sample_demand(demand_model, state, n) / n for _ in range(1000)]
...
queue_L15 = create_queue_allocation(L=15)
queue_L8 = create_queue_allocation(L=8)
queue_L4 = create_queue_allocation(L=4)
linear = create_linear_allocation()

x_vals = np.linspace(0.01, 2.5, 100)

print("Sample values:")
for x in [0.3, 0.5, 1.0, 1.5, 2.0]:
    print(f"  x = {x:.1f}: ω_L8(x) = {compute_omega(queue_L8, x):.4f}, "
          f"ω_L4(x) = {compute_omega(queue_L4, x):.4f}")
...
for L in [4, 8, 15]:
    alloc = create_queue_allocation(L=L)
    axes[0].plot(x_vals, [compute_omega(alloc, x) for x in x_vals], label=f'L={L}')
axes[0].plot(x_vals, [compute_omega(linear, x) for x in x_vals], '--', label='Linear')
...
axes[1].plot(x_vals, [compute_omega_derivative(queue_L15, x) for x in x_vals], label='L=15')
axes[1].plot(x_vals, [compute_omega_derivative(queue_L8, x) for x in x_vals], label='L=8')
axes[1].plot(x_vals, [compute_omega_derivative(queue_L4, x) for x in x_vals], label='L=4')
...
for b in [10, 20, 30, 40]:
    probs = [choice_fn(x, b) for x in x_vals]
...
avg_probs = [compute_activation_probability(x, supplier_params) for x in x_vals]
...
for rev in [10, 20, 30, 40, 50]:
    prob = compute_activation_probability(rev, supplier_params)
...
x_opt = d_a / opt_mu
q_opt = compute_omega(allocation, x_opt)
...
q_range = [compute_omega(allocation, x) for x in x_range]
demand_served = mu_range * np.array(q_range)

axes[1, 0].plot(payments, [m * compute_omega(allocation, d_a/m) for m in mus], 'b-', linewidth=2)
...
revenues = [gamma * m * compute_omega(allocation, d_a/m) for m in mus]
costs = [payments[i] * mus[i] * compute_omega(allocation, d_a/mus[i]) for i in range(len(payments))]
