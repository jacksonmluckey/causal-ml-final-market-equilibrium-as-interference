...
2. Each supplier learns the payment policy $\pi$ and their payment $p_i$ and decides whether to be active using their choice function $f_{B_i}$.
3. The platform randomly allocates demand among active suppliers using the allocation function $\Omega$.

In the simplest case, the platform receives $\gamma$ for each unit of demand served. The platform's utility is the sum of these payments minus the sum of the payments made to the suppliers. The goal of experimentation is to find the optimal payment $p^*$ at which the difference between overall revenue and payments to suppliers is maximized.
